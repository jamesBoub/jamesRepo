...
for i = 1,16 do
		...
		table.insert(ring, {x = target.x + math.cos(theta + angleOffset) * radius, y = target.y + math.sin(theta + angleOffset) * radius, angle = theta + angleOffset})
...
function love.draw()
	...
	for i in pairs(ring) do
		ring[i].x = target.x + math.cos(ring[i].angle) * radius
		ring[i].y = target.y + math.sin(ring[i].angle) * radius
